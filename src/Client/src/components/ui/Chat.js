...
function Chat({
  ...
  const chatMessages = messages !== undefined ? messages : internalMessages;
  const setChatMessages = onMessagesChange || setInternalMessages;
  const useExternalMessages = messages !== undefined;

  // Only set up socket listener when NOT using external messages
  useEffect(() => {
    if (useExternalMessages) {
      // External messages - parent handles socket, don't set up listener
      return;
    }
